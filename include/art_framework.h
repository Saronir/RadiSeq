...
public:
    ...
    void initialize_worker_storage(int);                                                               // Allocate per-worker scratch buffers once before launching worker threads
    ...
    static bool generate_paired_reads_with_indel(ART&, ART&, int, std::vector<double>&, int);           // Legacy function using ART's shared chromosome state
    static bool generate_paired_reads_with_indel(const radiseq::SegmentContext&, ART&, ART&, int, const std::vector<double>&, int); // Context-based paired generation for the queue pipeline
